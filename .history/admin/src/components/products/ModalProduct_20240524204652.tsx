...
export const ModalProduct: React.FC<ModalProps> = ({
  ...
  // const handleProductImageUpload = (key: keyof IProduct, e: any) => {
  //   const file = e.target.files[0];
  //   transformFile(key, file);
  // };

  // const transformFile = (key: keyof IProduct, file: any) => {
  //   const reader = new FileReader();

  //   if (file) {
  //     reader.readAsDataURL(file);
  //     reader.onloadend = () => {
  //       if (Array.isArray(dataProduct[key])) {
  //         changeProductValue(key, [...dataProduct[key], reader.result]);
  //       } else {
              ...
              if (key === "imgUrl") {
                const initialFileList = initialProductData?.imgUrl
                  ? initialProductData.imgUrl.map((url, index) => ({
                      ...
                      status: "done" as UploadFileStatus, // Explicitly casting to UploadFileStatus
                      url,
                    }))
                  : [];
                return (
