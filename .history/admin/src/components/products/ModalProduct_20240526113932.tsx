...
export const ModalProduct: React.FC<ModalProps> = ({
  ...
  const handleProductImageUpload = (key: string, fileList: UploadFile[]) => {
    ...
    fileList.forEach((file) => {
      const imgFile = file?.originFileObj;
      if (imgFile) {
        transformFile(imgFile, (result: string) => {
          newImages = result;
          setImages((prev: any) => [...prev, result]);
          console.log("ezezezezezeezezeeez");

          console.log(images);
        });
      }
    });
    changeProductValue(key, [...images, ...newImages]);
  };

  const transformFile = (file: any, callback: (result: string) => void) => {
