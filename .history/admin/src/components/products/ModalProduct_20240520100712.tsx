...
export const ModalProduct: React.FC<ModalProps> = ({
  ...
  const [dataProduct, setDataProduct] = useState({
    ...
    qte: initialProductData?.qte || "",
    price: initialProductData?.price || "",
    deliveryDate: initialProductData?.deliveryDate || "",
        ...
        <form className="space-y-8">
          <div className="grid grid-cols-2 gap-5">
            {Object.entries(params).map(([key, value]) => {
              if (key === "_id" || value || !(key in dataProduct)) return null;
              const fieldTitle = key[0].toUpperCase() + key.sclice(1);
