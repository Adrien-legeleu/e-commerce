...
export const Products: React.FC<ProductsProps> = () => {
  ...
  const [recipeToEdit, setRecipeToEdit] = useState<IProduct | null>();
  ...
  const updateProduct = (productProperties: IProduct, productId: string) => {
    setProducts((prev: any) => {
      return prev.map((product: IProduct) => {
      ...
      {products.map((product, index) => (
        <div key={`product number : ${index}`}>
          ...
          <ul>
            ...
            <li>{product.color}</li>
            <li>{product.price}</li>
            <li>{new Date(product.deliveryDate).toLocaleDateString()}</li>
          </ul>
          <div>
            <button
